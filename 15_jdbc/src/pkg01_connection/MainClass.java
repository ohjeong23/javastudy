...
public class MainClass {
   ...
   * 1.오라클 데이터 베이스에 접속하기 위해서 필요한 드라이버 클래스이다.
   * 2. 오라클에서 만들어서 베포한다.
   * 모든 드라이버 클래스는 따로 있는데 사용하는 디비에 따라 다른 드라이버클래스를 준비해야함
   * 3.18c xe 버전이면 C:\app\GD\product\18.0.0\dbhomeXE\jdbc\lib\ojdbc8.jar 파일을 프로젝트의 build path에 등록해야 한다.
   */
  
  /*
   * java.sql.Connection
   * 1. 데이터베이스 접속 객체 타입의 인터페이스이다.
   * 2. java.sql.DriiverManager클래스가 데이터베이스 접속 객체를 생성한다.
   * 3. 형식
  ...
  public static void method1() {
    ...
    //orcale.jdbc.OracleDriver 클래스 로드하기(메모리에 올리기)
    try {
